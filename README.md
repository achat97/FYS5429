...
**Note:** several intermediate files are **not included** in this repository because they are too large to upload, and must be generated locally before training or evaluation (see below):

- `ocean_noise_segments.npy` — the segmented and normalized noise, produced by `wav_spectogram.py`.
- `INPUT.npy` — the array of input spectrograms, produced by `pulses.py`.
- `best_model_epoch_20_valloss_0.3393.pth` — the trained model checkpoint, produced by `train_model.py`.
...
Run `evaluate.py` to evaluate the trained model on the test set and generate all result figures (detection, classification, regression, and pulse-count). `plot_examples.py` produces the example spectrogram shown in the report.

The remaining files are used as libraries: `model.py` contains the network architecture, the forward pass, and the masked loss; `functions.py` and `metrics_core.py` contain helper routines and the evaluation metrics; and `plots.py` contains the plotting functions called during evaluation.
